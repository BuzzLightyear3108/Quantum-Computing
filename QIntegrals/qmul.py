...
def setInputs(A, B, printResult=False):
    ...
    if A < 0: mCirc.data[0].operation.name = 'x'
    else: mCirc.data[0].operation.name = 'id'

    if B < 0: mCirc.data[inputDigits+1].operation.name = 'x'
    else: mCirc.data[inputDigits+1].operation.name = 'id'
...
# Set limit to 2**inputDigits, depending on inputDigits
# Here, I set the inputDigits to 2**20, which is well over 1 million
inputDigits = 20
